  ...
  return (
    <>
        <div className="flex items-center w-11/12 py-5 md:py-9 m-auto gap-5">
          <div>
            <button className="flex p-3 bg-blue-300/80" onClick={prev}>◀</button>
          </div>
          <div className="flex flex-wrap gap-8 justify-center">
            {citiesSlides.map((city, index) => (
              ...
              <img
                className="w-full h-full m-auto object-cover rounded-lg"
                src={city.image}
                alt={"Imagen de: " + city.name}
              />
              <footer className="w-full p-1 absolute bottom-0 left-0 right-0 m-auto rounded-b-lg text-white bg-gray-800/60">
          ...
          <div>
            <button className="flex p-3 bg-blue-300/80" onClick={next}>▶</button>
          </div>
        </div>
    </>
  );
