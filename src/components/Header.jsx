        ...
        <header className='flex justify-center border-b-2 border-b-black'>
            <div className='flex justify-between p-5 py- md:py-10 items-center w-[95%] sm:w-[80%]'>
              ...
              <nav className='flex items-center gap-6 lg:gap-10'>
                {links.map((link) => (
                  <Link to={link.path} key={link.title} className={`text-xl hover:scale-95 px-1 hover:text-orange-500 ${link.active ? "border-b-[3px] border-orange-700 hover:text-black hover:scale-100" : ""}`}>{link.title}</Link>
                ))}
